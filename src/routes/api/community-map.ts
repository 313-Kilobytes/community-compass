    ...
    handlers: {
      GET: async ({ request }: { request: Request }) => {
        ...
        try {
          ...
          const serpKeys = serpApiKeys();
          if (serpKeys.length === 0) {
            const places = await searchOpenStreetMapPlaces("all", lat, lng).catch(() => []);
            cache.set(cacheKey, {
              at: Date.now(),
              data: places,
              provider: "openstreetmap-overpass",
            });
            return Response.json({ places, provider: "openstreetmap-overpass" });
          }

          const serpPlaces = await searchWithSerpApiKeys(serpKeys, (key) =>
            fetchSerpNearbyPlaces(key, lat, lng),
          ).catch(() => searchOpenStreetMapPlaces("all", lat, lng).catch(() => []));
          cache.set(cacheKey, {
            at: Date.now(),
            data: serpPlaces,
            provider: serpPlaces.length ? "serpapi-google-maps" : "openstreetmap-overpass",
          });
          return Response.json({ places: serpPlaces, provider: serpPlaces.length ? "serpapi-google-maps" : "openstreetmap-overpass" });
        } catch (error) {
          return Response.json(
